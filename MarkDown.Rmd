...
k <- kable(files_zipped_df,format="html")

column_spec(k,1:3,border_left = T, border_right = T) %>% 
  kable_styling()
```
...
k <- kable(files_unzipped_df,format="html")

column_spec(k,1:2,border_left = T, border_right = T) %>% 
  kable_styling()
```
...
* We can see we will need to do some cleaning here, change "Subscriber" & "member"  to "Member" and "casual" & "Customer" to "Casual". Also remove "Dependents". We should also change it to factor data type.
